...
export default async function AdminDashboard() {
  ...
  const [{ count: pageCount }, { count: publishedCount }] = await Promise.all([
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '16px', marginBottom: '36px' }}>
        {stats.map(s => (
          <div key={s.label} className="card" style={{ padding: '24px' }}>
            <div style={{ width: '40px', height: '40px', background: 'var(--accent-subtle)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', marginBottom: '16px' }}>{s.icon}</div>
            <div style={{ fontSize: '32px', fontWeight: 800, letterSpacing: '-0.04em', color: 'var(--accent)', marginBottom: '4px' }}>{s.value}</div>
